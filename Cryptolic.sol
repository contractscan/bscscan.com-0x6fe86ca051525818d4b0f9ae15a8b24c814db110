...
interface IhsDistrib {
    function process() external;
}

interface IrwdDistrib {
 ...
 */
contract Cryptolic is IERC20MetadataUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    address DEAD;
    address ZERO;
    address public tokenLiquidator;
    address public tokenReserve;

    uint256 public tokenSupply;
    uint256 public contractSupply;
    ...
    bool public feesOnXfer;
    bool public freeze_contract;

    uint256 public distribGas;
    IrwdDistrib public rwdDistrib;
    IhsDistrib public hsDistrib;
    ...
    function initialize() public initializer {
        // CHECK SETTINGS BEFORE DEPLOYMENT
        __ERC20_init("Cryptolic", "CPTLC");
        __Ownable_init();
        ...
        tokenLiquidator = owner();
        tokenReserve = owner();

        tokenSupply = 1000000000000000 * (10 ** 18);
        contractSupply = tokenSupply.div(10);
        _maxTxAmount = tokenSupply.div(200);
        ...
        busdThreshold = 10 * (10 ** 18);

        feesOnXfer = false;
        freeze_contract = true;
        ...
        isFreezeExempt[owner()] = true;

        isFeeExempt[owner()] = true;
        isFeeExempt[address(this)] = true;
        isFeeExempt[tokenLiquidator] = true;

        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[tokenLiquidator] = true;
        isTxLimitExempt[address(this)] = true;

        isRwdExempt[bnbPair] = true;
        isRwdExempt[address(this)] = true;
        isRwdExempt[DEAD] = true;
        isRwdExempt[owner()] = true;
        isRwdExempt[tokenLiquidator] = true;

        approve(address(pcsRouter), tokenSupply);
        IERC20(BUSD).approve(address(pcsRouter), ~uint256(0));
        _mint(owner(), tokenSupply.sub(contractSupply));
        _mint(address(this), contractSupply);
    }

    /*
     * Basic ERC-20 Configurations
     */
    receive() external payable {}

    function _authorizeUpgrade(address newImplementaion) internal override onlyOwner {}
    function __ERC20_init(string memory name_, string memory symbol_) internal onlyInitializing { __ERC20_init_unchained(name_, symbol_); }
    ...
    function _approve(address owner_, address spender_, uint256 amount_) internal virtual {
        require(owner_ != address(0) && spender_ != address(0), "Approve the zero address");
    ...
    function _spendAllowance(address owner_, address spender_, uint256 amount_) internal virtual {
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount_, "Insufficient allowance");
            unchecked {
    ...
    function _transfer(address sender_, address recipient_, uint256 amount_) internal virtual {
        require(sender_ != address(0) && recipient_ != address(0), "Zero address");
        require(!freeze_contract || isFreezeExempt[sender_], "Contract frozen!");
        require(!checkTeamWallet(sender_, recipient_), "Team wallet is locked");
        ...
        if(!launched() && recipient_ == bnbPair) {
            ...
            launch();
            _basicTransfer(sender_, recipient_, amount_);
            return;
        }

        if(sender_ == tokenLiquidator) {
            _basicTransfer(sender_, recipient_, amount_);
            return;
        }

        require(_balances[sender_] >= amount_, "Transfer amount exceeds balance");
        ...
        unchecked {
            ...
            _balances[recipient_] += amountReceived;
        }

        uint256 swapAmount = amount_.sub(amountReceived);
        if(swapAmount > 0) {
            _basicTransfer(address(this), tokenLiquidator, swapAmount);
            uint256 burnAmount = swapAmount.div(2);
            _basicTransfer(address(this), DEAD, burnAmount);
        }

        if(!isRwdExempt[sender_]){ try rwdDistrib.setShare(sender_, _balances[sender_]) {} catch {} }
        if(!isRwdExempt[recipient_]){ try rwdDistrib.setShare(recipient_, _balances[recipient_]) {} catch {} }

        if(launched()) {
            if(shouldSendBUSD()) { sendBUSD(); }
            if(processHS) {
                try hsDistrib.process() {} catch {}
                processHS = false;
            }
            else {
                try rwdDistrib.process(distribGas) {} catch {}
                processHS = true;
            }
        }
    ...
    function _basicTransfer(address sender_, address recipient_, uint256 amount_) internal {
    ...
    function checkTeamWallet(address sender_, address recipient_) internal view returns (bool) {
        if (isTeamWallet[sender_] && recipient_ != tokenReserve) return true;
        return false;
    }
    ...
    function shouldSendBUSD() internal view returns (bool) {
        return IERC20(BUSD).balanceOf(address(this)) > busdThreshold;
    }
    function sendBUSD() internal {
        uint256 sendBUSDAmount = IERC20(BUSD).balanceOf(address(this));
        IERC20(BUSD).transfer(address(rwdDistrib), sendBUSDAmount);
        try rwdDistrib.depositBUSD(sendBUSDAmount) {} catch {}
    }

    /*
     * Contract Settings
     */
    function clearStuckBNB(address wallet_, uint256 amount_) external onlyOwner {
    ...
    function setTokenReserve(address wallet_) external onlyOwner {
        tokenReserve = wallet_;
    }

    function setTokenLiquidator(address wallet_) external onlyOwner {
        tokenLiquidator = wallet_;
    }

    function setTeamWallet(address wallet_, bool islocked_) external onlyOwner {
        isTeamWallet[wallet_] = islocked_;
    }

    function setTxLimit(uint256 amount_) external onlyOwner {
        _maxTxAmount = amount_;
    }

    function setIsRwdExempt(address holder_, bool exempt_) external onlyOwner {
        require(holder_ != address(this) && holder_ != bnbPair);
        isRwdExempt[holder_] = exempt_;
        if(exempt_){
            rwdDistrib.setShare(holder_, 0);
        }else{
            rwdDistrib.setShare(holder_, _balances[holder_]);
        }
    }

    function setDistribs(address rwdDistrib_, address hsDistrib_) external onlyOwner {
        rwdDistrib = IrwdDistrib(rwdDistrib_);
        hsDistrib = IhsDistrib(hsDistrib_);
    }
    ...
    function setFees(uint256 rewardFee_, uint256 hotshotFee_, uint256 feeDenominator_) external onlyOwner {
        rewardFee = rewardFee_;
        hotshotFee = hotshotFee_;
        totalFee = rewardFee_.add(hotshotFee_);
        feeDenominator = feeDenominator_;
        uint256 effectiveTax = totalFee.mul(100).div(feeDenominator);
        require(effectiveTax <= 15, "Taxes are set too high");
    }

    function setBUSDThreshold(uint256 amount_) external onlyOwner {
        busdThreshold = amount_;
    }

    function setDistribGas(uint256 distribGas_) external onlyOwner {
